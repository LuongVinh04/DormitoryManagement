...
export const ENTITY_CONFIGS = {
  ...
  roomZones: {
    ...
    fields: [
      ...
      { name: 'managerName', label: 'Người phụ trách', type: 'lookup', lookup: 'users', allowEmpty: true, optionValue: (item) => item.fullName || item.username, optionLabel: (item) => `${item.fullName || item.username} (${item.roleName || 'User'})` },
      { name: 'description', label: 'Mô tả', type: 'textarea', allowEmpty: true },
      { name: 'isActive', label: 'Đang sử dụng', type: 'checkbox' },
    ],
  },
  ...
  roomFinances: {
    ...
    fields: [
      ...
      { name: 'paidDate', label: 'Ngày thanh toán', type: 'date', allowEmpty: true, editOnly: true },
      { name: 'status', label: 'Trạng thái', type: 'select', options: INVOICE_STATUS_OPTIONS, editOnly: true },
      { name: 'paymentMethod', label: 'Hình thức thu', type: 'lookupCode', lookup: 'paymentMethods', allowEmpty: true, editOnly: true, optionLabel: (item) => `${item.name}${item.processingFee ? ` - phí ${currency.format(item.processingFee)}` : ''}` },
      { name: 'recordedBy', label: 'Người thu', type: 'lookup', lookup: 'users', allowEmpty: true, editOnly: true, optionValue: (item) => item.fullName || item.username, optionLabel: (item) => `${item.fullName || item.username} (${item.roleName || 'User'})` },
      { name: 'paymentNote', label: 'Ghi chú thu tiền', type: 'textarea', allowEmpty: true, editOnly: true },
    ],
  },
  invoices: {
    title: 'Hóa đơn',
