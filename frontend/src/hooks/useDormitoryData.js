...
export function useDormitoryData() {
  ...
  const activeRoomId =
    selectedRoomId || (data.rooms[0] ? String(data.rooms[0].id) : "");
  ...
  async function loadAll({ showLoading = false } = {}) {
    try {
      ...
      const [dashboardRes, financeSummaryRes, lookupRes, ...entityResponses] =
        await Promise.all([
      ...
      if (
        !dashboardRes.ok ||
        !financeSummaryRes.ok ||
        !lookupRes.ok ||
        entityResponses.some((res) => !res.ok)
      ) {
        throw new Error("Không thể tải dữ liệu hệ thống.");
      }

      const [dashboardJson, financeSummaryJson, lookupJson, ...entityJson] =
        await Promise.all([
          dashboardRes.json(),
  ...
  async function loadRoomOverview(roomId) {
    try {
      const response = await apiFetch(
        `/api/facilities/rooms/${roomId}/overview`,
      );
      if (!response.ok) {
  ...
  function openCreate(entityKey) {
    ...
    ENTITY_CONFIGS[entityKey].fields.forEach((field) => {
      ...
      } else if (field.type === "lookup" && !field.allowEmpty) {
        const items = lookups[field.lookup] ?? [];
        const firstItem = items[0];
        defaults[field.name] = firstItem
          ? String(
              field.optionValue ? field.optionValue(firstItem) : firstItem.id,
            )
          : "";
      } else if (field.type === "lookupCode" && !field.allowEmpty) {
        const items = lookups[field.lookup] ?? [];
        defaults[field.name] = items[0]?.code ?? "";
      } else {
        defaults[field.name] = "";
      }
    });
      ...
      values[field.name] =
        field.type === "date"
          ? rawValue
            ...
            : ""
          : (rawValue ?? (field.type === "checkbox" ? false : ""));
    });
  ...
  function updateModalField(name, value) {
    setModal((current) => {
      ...
      if (current.entityKey === "rooms" && name === "roomCategoryId") {
        const category = lookups.roomCategories.find(
      ...
      if (
        current.entityKey === "students" &&
        name === "studentCode" &&
        !nextValues.accountUsername
      ) {
        nextValues.accountUsername = String(value || "")
          .trim()
          .toLowerCase();
      }

      if (current.entityKey === "rooms" && name === "roomZoneId") {
        const zone = lookups.roomZones.find(
          (item) => String(item.id) === String(value),
        );
        if (zone?.buildingId) {
      ...
      if (current.entityKey === "utilities" && name === "roomId") {
        const profile = data.roomFeeProfiles.find(
          (item) => String(item.roomId) === String(value),
        );
        if (profile) {
      ...
      if (current.entityKey === "roomFinances" && name === "utilityId") {
        ...
        if (utility) {
          ...
          nextValues.billingMonth =
            utility.billingMonth ?? nextValues.billingMonth;
          if (utility.roomId && !nextValues.roomId) {
            nextValues.roomId = String(utility.roomId);
            const profile = data.roomFeeProfiles.find(
              (item) => String(item.roomId) === String(utility.roomId),
            );
            if (profile) {
              nextValues.monthlyRoomFee = profile.monthlyRoomFee ?? 0;
              nextValues.hygieneFee = profile.hygieneFee ?? 0;
  ...
  async function saveEntity() {
    ...
    const endpoint =
      mode === "create"
        ? API_ENDPOINTS[entityKey]
        : `${API_ENDPOINTS[entityKey]}/${id}`;
    ...
    try {
      ...
      setNotice(
        mode === "create" ? "Đã thêm dữ liệu mới." : "Đã cập nhật dữ liệu.",
      );
      setModal(null);
      setFormErrors({});
      await refreshData();
  ...
  async function confirmDeleteAction() {
    ...
    try {
      ...
      setNotice("");
      const response = await apiFetch(
        `${API_ENDPOINTS[entityKey]}/${item.id}`,
        { method: "DELETE" },
      );
      if (!response.ok) {
        throw new Error(await readError(response));
